...
class MedicineRemoteDataSource {
  ...
  Future<List<Medicine>> searchMedicines(String query) async {
    final response = await _httpClient.get(Uri.parse('$ApiConstants.baseUrl/medicines/search?query=$query'));
    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.map((json) => MedicineModel.fromJson(json).toEntity()).toList();
    } else {
  ...
  Future<Medicine> getMedicineDetails(String id) async {
    final response = await _httpClient.get(Uri.parse('$ApiConstants.baseUrl/medicines/$id'));
    if (response.statusCode == 200) {
